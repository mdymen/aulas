...
ALTER TABLE `cursos` 
ADD COLUMN `FL_TIPO_CR` INT NULL DEFAULT 0 AFTER `ST_SUBTITULO_CR`;

ALTER TABLE `cursos` 
ADD COLUMN `ST_AUTOR_CR` VARCHAR(200) NULL AFTER `FL_TIPO_CR`;

ALTER TABLE `cursos` 
ADD COLUMN `FL_DISPONIVEL_CR` INT NULL DEFAULT 0 AFTER `ST_AUTOR_CR`;

ALTER TABLE `cursos` 
ADD COLUMN `FL_EXCLUIDO_CR` INT NULL DEFAULT 0 AFTER `FL_DISPONIVEL_CR`;

CREATE TABLE `compras` (
  `ID_ID_COM` INT NOT NULL AUTO_INCREMENT,
  `ID_USUARIO_COM` INT NULL,
  `ID_CURSO_COM` INT NULL,
  `DT_DATA_COM` INT NULL,
  `VL_PRECO_COM` DOUBLE NULL,
  PRIMARY KEY (`ID_ID_COM`));

ALTER TABLE `compras` 
CHANGE COLUMN `DT_DATA_COM` `DT_DATA_COM` DATETIME NULL DEFAULT NULL ;

ALTER TABLE `compras` 
CHANGE COLUMN `DT_DATA_COM` `DT_DATA_COM` DATE NULL DEFAULT NULL ;

CREATE TABLE `usuario_dados` (
  `ID_ID_USU` INT NOT NULL,
  `ST_NOME_USU` VARCHAR(100) NULL,
  `ST_CPF_USU` VARCHAR(45) NULL,
  `ST_BANCO_USU` VARCHAR(45) NULL,
  `ST_AGENCIA_USU` VARCHAR(45) NULL,
  `ST_CONTA_USU` VARCHAR(45) NULL,
  PRIMARY KEY (`ID_ID_USU`),
  UNIQUE INDEX `ST_CPF_USU_UNIQUE` (`ST_CPF_USU` ASC));

ALTER TABLE `usuarios` 
ADD COLUMN `ID_FACEBOOK_USU` VARCHAR(100) NULL AFTER `ST_CONFIRMADO_USU`;
